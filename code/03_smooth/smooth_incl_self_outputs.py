"""smooth_s8_incl_self: outputs + metrics + README + registry (reuse smooth_s8 logic)."""
import os
import numpy as np
import pandas as pd
import scanpy as sc
...
SID = "smooth_s8_incl_self"
OUT = os.path.join(RES, SID)
os.makedirs(os.path.join(OUT, "figures"), exist_ok=True)
os.makedirs(os.path.join(OUT, "tables"), exist_ok=True)

slim = sc.read_h5ad(os.path.join(OUT, "data", f"{SID}_slim.h5ad"))
...
def hungarian(labels):
    ...
    for i in range(n_ann):
        a = ann_cats[i]; j = ci[i]
        if j < n_cl:
            ov[a] = round(float(ct.loc[a, clusters[j]] / ct.loc[a].sum()), 3); ann2cl[a] = clusters[j]
        else:
            ov[a] = 0.0; ann2cl[a] = None
    return ct, ann2cl, ov

tmp = sc.AnnData(X=X_pca.copy())
tmp.obsp["connectivities"] = conn; tmp.obsp["distances"] = conn.copy()
...
for res in RESOLUTIONS:
    ...
    sil = silhouette_score(X_pca[sub], lab.astype(int)[sub]); coh = coherence(slim.obs[f"{SID}_r{rs}"])
    ...
    pd.DataFrame([{"annotation": a, "cluster": ann2cl[a], "overlap_fraction": ov[a]} for a in ann_cats]
                 ).to_csv(os.path.join(OUT, "tables", f"matching_r{rs}.csv"), index=False)
    ...
    for a in ann_cats:
        m = (ann == a); axes[0].scatter(xy[m, 0], xy[m, 1], s=2, color=ann_colors[a], rasterized=True)
    ...
    for cl in list(ct.columns):
        m = (lab == cl); axes[1].scatter(xy[m, 0], xy[m, 1], s=2, color=cl2col[cl], rasterized=True)
    axes[1].set_title(f"{SID} (res={rs})"); axes[1].set_aspect("equal"); axes[1].invert_yaxis()
...
for a in ann_cats:
    m = (ann == a); axes[0].scatter(um[m, 0], um[m, 1], s=2, color=ann_colors[a], rasterized=True)
...
print("=== smooth_s8_incl_self overall metrics ===")
print(mdf.to_string(index=False))

lines = [f"# 方案：{SID}", "", "## 参数",
         "- 聚类算法：Leiden", "- PCA求解器：arpack", "- PCA维度：30",
         "- e_neigh：30", "- s_neigh：8（平滑用空间邻居数）",
         "- 是否用了空间约束：是（表达层空间平滑，含自身）",
         "- 空间信息利用方式：高斯核（σ=中位邻居距离）对 log1p 表达做「自身+8邻居」加权平均（自身距离0权重最大），再走全基因PCA(arpack-30)→neighbors(e_neigh=30)→Leiden", "",
         "## 各分辨率关键指标",
...
lines += ["", "## 本方案产出文件清单",
          ...
          "- data/: smooth_s8_incl_self_slim.h5ad", "",
          "## 已知问题/待办", "含自身版本，用于对照排除自身版(smooth_s8)是否过度激进。"]
