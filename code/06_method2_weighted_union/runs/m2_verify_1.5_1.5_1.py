...
lab = F.leiden_w(adj, 1.5, 1)
n_pre = int(lab.max()) + 1
lab2, sc_own_pre, triggered = F.sc_split(lab, ann)
np.save('F:/tmp/m2_final_corrected_alpha1.5_res1.5_seed1.npy', lab2)
m = F.metrics(lab2, a, ann_s, sub)
ov = m['overlaps']
sha = hashlib.sha256(lab2.astype(np.int32).tobytes()).hexdigest()[:16]
print(json.dumps({"alpha":1.5,"res":1.5,"seed":1,"n_clusters_pre":n_pre,"n_clusters_post":m["n_clusters"],"sc_split_triggered":triggered,"sc_own_pre":sc_own_pre,"sc_overlap_post":ov["Spinal cord"],"drg_overlap_post":ov["Dorsal root ganglion"],"ari":m["ari"],"nmi":m["nmi"],"silhouette":m["silhouette"],"coherence":m["coherence"],"label_sha256":sha,"overlaps":ov}))
